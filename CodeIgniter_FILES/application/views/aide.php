

<h2 align='center'> Sommaire </h2>
<ul>
<li><a href="#notes">Ajouter/Modifier/Supprimer des notes</a></li>
<li><a href="#notes_ant">Afficher les notes des années précédentes</a></li>

<li><a href="#etu_aj">Ajouter un étudiant</a></li>
<li><a href="#etu_mo">Modifier les informations personnelles d'un étudiant</a></li>
<li><a href="#etu_su">Supprimer un étudiant</a></li>

<li><a href="#PV">Imprimer un PV</a></li>
<li><a href="#bilan">Imprimer un ou tout les bilans de notes</a></li>
</ul>

<br/><br/><hr><hr><br/><br/>

<ul>
<li><a name="notes"><strong> Ajouter/Modifier/Supprimer des notes</strong></a></li>
<p>
	1/ Aller dans "Gérer les unités d'enseignement" <br/>
	2/ Choisir l'année scolaire voulue <br/>
	3/ Sur la ligne de l'ue contenant les notes à ajouter/modifier/supprimer,  cliquer sur "voir les notes". <br/>
	4/ Une fois les modifications effectuées, cliquer sur enregistrer. </a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="notes_ant"><strong> Afficher les notes des années précédentes </strong></a></li>
<p>
	1/ Aller dans "Gérer les unités d'enseignement" <br/>
	2/ Cliquer sur "Voir les notes" de l'ue choisie <br/>
	3/ Dans le menu déroulant "Choix de l'année", choisir l'année voulue <br/>
	4/ Cliquer sur "Afficher les notes". </a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="etu_aj"><strong> Ajouter un étudiant </strong></a></li>
<p>
	1/ Aller dans "Gérer les étudiants"
	2/ Cliquer sur Ajouter un étudiant
	3/ Compléter le formulaire. 
	L'étudiant sera inscrit à toutes les Unités d'enseignement cochées.</a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="etu_mo"><strong> Modifier un étudiant </strong></a></li>
<p>
	1/ Aller dans "Gérer les étudiants" 
	2/ Cliquer sur "Modifier" sur la ligne de l'étudiant voulu
	3/ Compléter le formulaire. </a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="etu_su"><strong> Supprimer un étudiant </strong></a></li>
<p>
	1/ Aller dans "Gérer les étudiants" 
	2/ Cliquer sur "Supprimer" sur la ligne de l'étudiant voulu </a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="PV"><strong>Imprimer un PV</strong></a></li>
<p>
	1/ Aller dans "Gérer les unités d'enseignement"
	2/ Choisir le niveau et l'année de passage voulus
	3/ Cliquer sur créer le PV.
	4/ Télécharger, puis imprimer. </a> 
</p>

<br/><br/><hr><hr><br/><br/>

<li><a name="bilan"><strong> Imprimer un ou tout les bilans de notes </strong></a></li>
<p>
	1/ Aller dans "Gérer les étudiants".
	2/ 
		A) Tout les bilans de notes :
			a/ Cliquer sur "Bilan de notes de étudiants en PDF"
			b/ Télécharger, et imprimer.
	
		B) U n bilan de notes :
			a/ Choisir l'étudiant dans la liste
			b/ Cliquer sur "Voir les notes"
			c/ Télécharger et imprimer </a> 
</p>
</ul>
